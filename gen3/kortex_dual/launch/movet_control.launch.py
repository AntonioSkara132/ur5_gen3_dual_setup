...
def launch_setup(context, *args, **kwargs):
    ...
    dual_setup = LaunchConfiguration("dual_setup")
    
    rd_path = PathJoinSubstitution([robot_namespace, 'robot_description'])
    js_path = PathJoinSubstitution([robot_namespace, 'joint_states'])
    ...
    joint_state_broadcaster_spawner = Node(
        ...
        remappings=[('/joint_states', js_path)],
        namespace=robot_namespace
    )
    ...
    robot_traj_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[robot_traj_controller, "-c", controller_manager_path],
        namespace=robot_namespace,
    )
    ...
    robot_pos_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[robot_pos_controller, "--inactive", "-c", controller_manager_path],
        namespace=robot_namespace,
    )
    ...
    #     package="controller_manager",
    #     executable="spawner",
    #     arguments=[robot_hand_controller, "-c", controller_manager_path],
    ...
    #     package="controller_manager",
    #     executable="spawner",
    #     arguments=[robot_lite_hand_controller,"-c", controller_manager_path],
    ...
    bridge = Node(
        ...
        output="screen",
        condition=IfCondition(dual_setup),
        namespace=robot_namespace,
    )
    ...
    gazebo_bridge = Node(
        ...
        # arguments=[
        #     "/kinova/wrist_mounted_camera/image@sensor_msgs/msg/Image[gz.msgs.Image",
        #     "/kinova/wrist_mounted_camera/depth_image@sensor_msgs/msg/Image[gz.msgs.Image",
        #     "/kinova/wrist_mounted_camera/points@sensor_msgs/msg/PointCloud2[gz.msgs.PointCloudPacked",
        #     "/kinova/wrist_mounted_camera/camera_info@sensor_msgs/msg/CameraInfo[gz.msgs.CameraInfo",            
        # ],
        namespace=robot_namespace,
        output="screen",
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument("namespace", default_value="", description="Namespace for robot nides")
    )

    declared_arguments.append(
        DeclareLaunchArgument("robot_description_content", default_value="", description="Robot description")
    )
    declared_arguments.append(
        DeclareLaunchArgument("dual_setup", default_value="true", description="Dual or singular setup")
    )
